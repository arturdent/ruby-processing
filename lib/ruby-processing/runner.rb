  ...
  class Runner
    ...
    def create(sketch, args)
      ...
      if @options.wrap
        case args.length
        when 0
          Processing::ClassSketch.new.create!(name: sketch)
        when 2
          Processing::ClassSketch.new.create!(name: sketch, width: args[0], height: args[1])
        else
          Processing::ClassSketch.new.create!(name: sketch, width: args[0], height: args[1], mode: args[2])
        end
        return
      end
      case args.length
      when 0
        Processing::BasicSketch.new.create!(name: sketch)
      when 2
        Processing::BasicSketch.new.create!(name: sketch, width: args[0], height: args[1])
      else
        Processing::BasicSketch.new.create!(name: sketch, width: args[0], height: args[1], mode: args[2])
      end
    end
